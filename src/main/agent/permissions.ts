...
export function needsApproval(
  actionText: string,
  opts: { mode: 'normal' | 'detached'; askBeforeSensitive: boolean }
): ApprovalDecision {
  if (opts.mode === 'normal' && opts.askBeforeSensitive) {
    return { needsApproval: true, reason: 'Local-PC actions always need approval in v1.' }
  }
  if (opts.askBeforeSensitive && isSensitiveAction(actionText)) {
    return { needsApproval: true, reason: 'Sensitive action detected.' }
  }
